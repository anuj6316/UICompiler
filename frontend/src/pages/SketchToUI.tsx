...
import { ResultPanel } from '../features/sketch-workspace/components/ResultPanel';
...
export default function SketchToUI() {
  ...
  const { generated, handleGenerate } = genState;
    ...
    <DashboardLayout
      header={({ setIsMobileMenuOpen }) => (
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center bg-zinc-100 dark:bg-[#1a1a1f] p-1 border border-zinc-200 dark:border-white/[0.08] rounded-none shadow-sm dark:shadow-none mr-2">
            <Button size="icon" variant="ghost" onClick={undo} className="h-8 w-8 rounded-none text-zinc-500 hover:text-zinc-900 dark:hover:text-white"><Undo className="w-4 h-4" /></Button>
            <Button size="icon" variant="ghost" onClick={redo} className="h-8 w-8 rounded-none text-zinc-500 hover:text-zinc-900 dark:hover:text-white"><Redo className="w-4 h-4" /></Button>
          </div>
          
          <div className="hidden md:flex items-center gap-2 mr-2">
            <Button variant="ghost" size="sm" className="text-xs font-bold uppercase tracking-widest h-9 rounded-none text-zinc-500 hover:text-zinc-900 dark:hover:text-white px-4">
          ...
          <Button 
            onClick={() => {
              if (canvasState.elements.length > 0) {
                const lastSketch = canvasState.elements[canvasState.elements.length - 1];
                handleGenerate();
                
                // Add a result node to the canvas
                const resId = `result-${Date.now()}`;
                const newResult: any = {
                  id: resId,
                  type: 'result',
                  x: (lastSketch.x || 0) + 600, // Position to the right
                  y: (lastSketch.y || 0),
                  sourceSketchId: lastSketch.id,
                  generationId: resId
                };
                canvasState.setElements([...canvasState.elements, newResult]);
                
                // Smoothly pan to the new result
                setTimeout(() => {
                  canvasState.setStagePos({
                    x: -(newResult.x * canvasState.stageScale) + (window.innerWidth / 2) - 320, // Center it (half of ResultPanel width)
                    y: -(newResult.y * canvasState.stageScale) + (window.innerHeight / 2) - 300
                  });
                }, 100);
            ...
            disabled={genState.generationState !== 'idle' && genState.generationState !== 'failed'}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-10 w-10 p-0 rounded-none overflow-hidden hover:bg-zinc-100 dark:hover:bg-[#1e1e24] transition-colors">
