...
export function CanvasArea({ canvasState, generated, children }: CanvasAreaProps) {
    ...
    <div ref={containerRef} className="flex-1 relative bg-background overflow-hidden cursor-crosshair">
      ...
      {stageSize.width > 0 && (
        ...
        >
          ...
          {canvasState.elements.filter((el: any) => el.type === 'result').map((el: any) => (
            <Layer key={`result-layer-${el.id}`}>
              <Html 
                divProps={{ 
                  style: { 
                    position: 'absolute', 
                    top: el.y || 50, 
                    left: el.x || 800,
                    transform: `scale(${canvasState.stageScale})`,
                    transformOrigin: '0 0'
                  } 
                }}
              >
                {/* ResultPanel will need props from el or shared context */}
                {children}
              </Html>
            </Layer>
          ))}
